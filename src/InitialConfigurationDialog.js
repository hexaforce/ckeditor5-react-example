      ...
      <div id="overlay" className={isWarning ? " warning" : ""}>
        <form className="body" onSubmit={(evt) => this.handleSubmit(evt)}>
          ...
          <div>
            ...
            <input name="web-socket-url" onChange={(evt) => this.handleConfigChange(evt.target.value, "webSocketUrl")} value={config.webSocketUrl} />
          </div>
          <div>
            <label htmlFor="token-url">Token URL</label>
            <input required name="token-url" onChange={(evt) => this.handleTokenUrlChange(evt.target.value)} value={config.tokenUrl} />
          </div>
          <div id="additional" className={isCloudServicesTokenEndpoint(config.tokenUrl) ? "visible" : ""}>
            ...
            <div id="user-container">
              {users.map((data) => {
                return (
                  <div key={data.id} onClick={() => this.selectUser(data)} className={selectedUser == data.id ? "active" : ""}>
          ...
          <div>
            ...
            <input name="ckbox-token-url" onChange={(evt) => this.handleConfigChange(evt.target.value, "ckboxTokenUrl")} value={config.ckboxTokenUrl} />
          </div>
          <div>
